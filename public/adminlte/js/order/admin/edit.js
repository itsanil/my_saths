  $(function () {
                ...
                <input type="hidden" class="form-control" name="product_id[]" id="exampleInputEmail1" value="'+data[0]+'" readonly="" >\
                    <div class="col">\
                        <div class="form-group text-center">\
                            ...
                            <input type="hidden" class="form-control" name="product_name[]" id="exampleInputEmail1" value="'+data[1]+'" readonly="" ><h4 class="" style="margin-top: 8px;">&nbsp;'+data[1]+'</h4>\
                        </div>\
                    </div>\
                    <div class="col">\
                        <div class="form-group text-center">\
                            <label for="exampleInputEmail1">Purchase Price :</label>\
                            <input type="hidden" class="form-control" name="" id="exampleInputEmail1" value="'+data[5]+'" readonly="" ><h4 class="" style="margin-top: 8px;">&nbsp;'+data[5]+'</h4>\
                        </div>\
                    </div>\
                    <div class="col">\
                        <div class="form-group text-center"><label for="exampleInputEmail1">Price :</label>\<input type="hidden" class="form-control" name="per_price[]" id="exampleInputEmail1" value="'+product_price+'" readonly="" >\
                            <h4  style="margin-top: 8px;">'+product_price+'</h4></div>\
                    </div>\
                    <div class="col">\
                    <div class="form-group text-center">\
                            <label for="exampleInputEmail1">Available Qty</label>\
                            <input type="hidden" class="form-control" name="available_qty[]" id="exampleInputEmail1" value="'+data[4]+'" readonly=""><h4  style="margin-top: 8px;">'+data[4]+'</h4>\
                        </div>\
                        </div>\
                    <div class="col">\
    ...
    $('#product_select2').on('change',function(){
        ...
        $('#cal_order_amount').on('click',function(){
            ...
            if($('#product_qty').val() != ''){
                var product_qty_array = $("input[name='product_qty[]']").map(function(){return $(this).val();}).get();
                var price_array = $("input[name='per_price[]']").map(function(){return $(this).val();}).get();
                $.each(product_qty_array, function( index, value ) {
                    if(value != ''){
                        // console.log(price_array);
                        // alert(price_array[index]);
